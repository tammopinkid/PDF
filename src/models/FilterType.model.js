...
const filter = jsonObject => {
  ...
  if (events.length > 0) {
    events.forEach(item => {
      ...
      const componentType = JSON.parse(item.payload).params.componentType;//JSON.parse(item.payload).params.componentType;//item.payload.params.componentType;
      ...
      if (isIgnore === false) {
        switch (componentType) {
          ...
          case 'YesNoList': {
          data2.push(Yesnolist.compile(item))
          console.log(data2)
          break
           }
          case 'Auto': {
          data.push(Auto.compile(item))
